...
export class AttendanceService {
  ...
  async findMyAttendance(requestingUser: any, filter: FilterAttendanceDto) {
    return this.findAttendance(
      { userId: requestingUser.sub },
      filter,
    );
  }

  async findCompanyAttendance(requestingUser: any, filter: FilterAttendanceDto) {
    const where: any = {
    ...
    const [totalEmployees, presentToday, lateToday, absentToday] = await Promise.all([
      ...
      this.prisma.attendanceDay.count({
        where: {
          ...
          status: { in: [AttendanceStatus.ON_TIME, AttendanceStatus.COMPLETED, AttendanceStatus.IN_PROGRESS] },
        },
      }),
      this.prisma.attendanceDay.count({
        where: {
